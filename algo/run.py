...
def train_all():
    ...
    model = MLP(784, 10, [64, 64, 64])
    print('training for 100 epochs')
    train('MLP_3_CEL_SGD', model, torch.nn.CrossEntropyLoss(), torch.optim.SGD(model.parameters(), lr = 0.001), 10, transform, train_data, train_loader)
...
def test_all():
    ...
    with open('performance.csv', mode='w') as performance:
        ...
        for epoch in range(10):
            write.writerow(['MLP_3_CEL_SGD_' + str(epoch), test('MLP_3_CEL_SGD_' + str(epoch), model, test_data, test_loader)])
