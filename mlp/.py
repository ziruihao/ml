...
class MLP(torch.nn.Module):
    ...
    def __init__(self, input_size, output_size, hidden_layer_sizes):
        ...
        for i, hidden_layer_size in enumerate(hidden_layer_sizes):
            construct_layers.append((str(i) + 'linear', torch.nn.Linear(prev_layer_size, hidden_layer_size)))
            construct_layers.append((str(i) + 'activation', torch.nn.ReLU()))
            prev_layer_size = hidden_layer_size
        construct_layers.append((str(len(hidden_layer_sizes)) + 'linear', torch.nn.Linear(prev_layer_size, output_size)))
        construct_layers.append(('final activation', torch.nn.LogSoftmax(dim = 1)))

        for layer in construct_layers:
            print(layer)

        self.layers = torch.nn.Sequential(OrderedDict(construct_layers))
    def forward(self, x):
